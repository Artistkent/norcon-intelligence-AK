...
export default function Sheet02Team({ data, locked, loginCodes, onUpdate }) {
  ...
  const removeMember = (idx) => {
    if (idx === 0) return; // cannot remove PM
    ...
    <div style={{ maxWidth:900 }}>
      ...
      {members.map((m,i)=>(
        <div key={i} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:7, padding:"12px 14px", marginBottom:8,
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
            ...
            {i > 0 && <button onClick={()=>removeMember(i)} style={{ marginLeft:"auto", background:"none", border:"none", color:C.muted, cursor:"pointer", fontSize:13 }}>✕</button>}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
            <div>
              <Lbl>Full Name</Lbl>
              <input style={{...inp}} value={m.name} disabled={i===0} onChange={e=>update(i,'name',e.target.value)} placeholder="Full name"/>
            </div>
            <div>
              <Lbl>PM / Governance Role</Lbl>
              <select style={{...inp}} value={m.role} disabled={i===0} onChange={e=>update(i,'role',e.target.value)}>
            ...
            <div>
              ...
              <select style={{...inp}} value={m.deliveryRole||''} onChange={e=>update(i,'deliveryRole',e.target.value)}>
                <option value="">Select...</option>
            ...
            <div>
              ...
              <select style={{...inp}} value={m.availability||'80%'} onChange={e=>update(i,'availability',e.target.value)}>
                {["100%","80%","60%","50%","Part-time","Ad hoc"].map(v=><option key={v} value={v} style={{background:C.surface2}}>{v}</option>)}
              </select>
            </div>
            <div>
              <Lbl>Location</Lbl>
              <input style={{...inp}} value={m.location||''} onChange={e=>update(i,'location',e.target.value)} placeholder="City / Remote"/>
            </div>
            <div>
              <Lbl>Primary Responsibilities</Lbl>
              <input style={{...inp}} value={m.responsibilities||''} onChange={e=>update(i,'responsibilities',e.target.value)} placeholder="Brief summary"/>
            </div>
          </div>
        </div>
      ))}
      ...
      <button onClick={addMember}
        style={{ padding:"8px 14px", background:"none", border:`1px dashed ${C.border}`,
          borderRadius:6, color:C.dim, fontSize:12, cursor:"pointer", width:"100%", marginTop:4, marginBottom:16 }}>
        + Add Team Member
      </button>

      <div style={{ marginTop:0, background:C.surface, border:`1px solid ${C.border}`, borderRadius:7, padding:"12px 16px" }}>
