...
const C = { surface:"#122E1E", surface2:"#183D28", border:"#1F4D34", accent:"#2E7D52", accentL:"#3a9962", sage:"#E5F0E8", dim:"#8aac96", muted:"#5a7a66", risk:"#e05c5c", opp:"#9c6ee0", milestone:"#e0a23a" };
...
// User type definitions for non-team external access
...
function generateLoginCode(type) {
...
export default function Sheet02Team({ data, locked, loginCodes, onUpdate }) {
  ...
  // External users (sponsors, guests, observers) — editable even when locked
  const [external, setExternal] = useState(data.externalUsers || []);
  const [copied,   setCopied]   = useState(null); // loginCode string just copied
  ...
  const addMember = () => {
    const code = `${(data.teamMembers?.[0]?.loginCode||"PM").replace(/[0-9]/g,"")}${String(Math.floor(Math.random()*9000)+1000)}`;
    const next = [...members, { loginCode:code, name:"", role:"", deliveryRole:"", availability:"80%", location:"", responsibilities:"", rights:[] }];
    setMembers(next);
    onUpdate({ teamMembers:next, externalUsers:external }, "in-progress");
  };
  ...
  const generateCode = (idx) => {
    ...
    const code  = generateLoginCode(user.type);
  ...
  return (
    <div style={{ maxWidth:900 }}>
      ...
      <div style={{ fontSize:11, color:C.accentL, marginBottom:10 }}>Team members can be added at any time. The PM login code and role cannot be changed after approval.</div>
      ...
      {members.map((m,i)=>(
        <div key={i} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:7, padding:"12px 14px", marginBottom:8,
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
            ...
            {i>0 && !locked && <button onClick={()=>removeMember(i)} style={{ marginLeft:"auto", background:"none", border:"none", color:C.muted, cursor:"pointer", fontSize:13 }}>✕</button>}
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
            <div><Lbl>Full Name</Lbl>
              <input style={inp} value={m.name} disabled={i===0||locked} onChange={e=>update(i,"name",e.target.value)} placeholder="Full name"/>
            </div>
            <div><Lbl>PM / Governance Role</Lbl>
              <select style={inp} value={m.role} disabled={i===0||locked} onChange={e=>update(i,"role",e.target.value)}>
                <option value="">Select...</option>
                {PM_ROLES.map(r=><option key={r} value={r} style={{background:C.surface2}}>{r}</option>)}
              </select>
            </div>
            <div><Lbl>Delivery / Technical Role</Lbl>
      ...
      {/* Always editable — sponsors and guests may be added before or after launch */}
          ...
          return (
            <div key={u.id||i} style={{ background:C.surface, border:`1px solid ${C.border}`,
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
                <div><Lbl>Name</Lbl>
                  <input style={inp} value={u.name||""} onChange={e=>updateExternal(i,"name",e.target.value)} placeholder={`${typeInfo.label} name`}/>
                </div>
                <div><Lbl>Access type</Lbl>
                  <select style={inp} value={u.type} onChange={e=>updateExternal(i,"type",e.target.value)}>
                    {EXTERNAL_TYPES.map(t=><option key={t.value} value={t.value} style={{background:C.surface2}}>{t.label} — {t.desc}</option>)}
                  </select>
                </div>
              </div>
              ...
              {/* Generate code button — always available */}
              <button onClick={()=>generateCode(i)}
