...
// APM BoK project lifecycle phases
const APM_PHASES = [
  "Concept",
  "Definition",
  "Development",
...
export default function Sheet03Schedule({ data, locked, loginCodes, onUpdate }) {
  ...
  // Build combined role list from team register + defaults
  ...
  const addActivity = () => {
    const next = [...activities, {
      ...
      name:"", phase:"", responsible:"", description:"", _state:"pending"
    }];
    setActivities(next);
    onUpdate({ activities:next, milestones }, 'in-progress');
  };
  ...
  const addMilestone = () => {
    const next = [...milestones, {
      ...
      name:"", phase:"", targetDate:"", description:"", _state:"pending"
    }];
    setMilestones(next);
    onUpdate({ activities, milestones:next }, 'in-progress');
  };
      ...
      {activities.map((a,i)=>(
        <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,padding:"10px 12px",marginBottom:8}}>
          <div style={{display:"grid",gridTemplateColumns:"90px 1fr 1fr 1fr auto",gap:8,alignItems:"end"}}>
            <div>
              <Lbl c="ID"/>
              <EditableField value={a._id} disabled={locked} onChange={v=>updateActivity(i,"_id",v)} placeholder="ACT-001"/>
            </div>
            <div>
              <Lbl c="Activity Name"/>
              <EditableField value={a.name} disabled={locked} onChange={v=>updateActivity(i,"name",v)} placeholder="Activity description"/>
            </div>
            <div>
              <Lbl c="APM Phase"/>
              <EditableSelect value={a.phase} disabled={locked} onChange={v=>updateActivity(i,"phase",v)} options={APM_PHASES} placeholder="Select phase..."/>
            </div>
            <div>
              <Lbl c="Responsible"/>
              <EditableSelect value={a.responsible} disabled={locked} onChange={v=>updateActivity(i,"responsible",v)} options={roleOptions} placeholder="Select role..."/>
            </div>
            {!locked && (
              <button onClick={()=>removeActivity(i)} style={{background:"none",border:"none",color:C.risk,cursor:"pointer",fontSize:16,paddingBottom:2,alignSelf:"end"}}>✕</button>
            )}
          </div>
      ...
      {milestones.map((m,i)=>(
        <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderLeft:`3px solid ${C.milestone}`,borderRadius:7,padding:"10px 12px",marginBottom:8}}>
          <div style={{display:"grid",gridTemplateColumns:"90px 1fr 1fr 160px auto",gap:8,alignItems:"end"}}>
            <div>
              <Lbl c="ID"/>
              <EditableField value={m._id} disabled={locked} onChange={v=>updateMilestone(i,"_id",v)} placeholder="MS-001"/>
            </div>
            <div>
              <Lbl c="Milestone Name"/>
              <EditableField value={m.name} disabled={locked} onChange={v=>updateMilestone(i,"name",v)} placeholder="Milestone name"/>
            </div>
            <div>
              <Lbl c="APM Phase"/>
              <EditableSelect value={m.phase} disabled={locked} onChange={v=>updateMilestone(i,"phase",v)} options={APM_PHASES} placeholder="Select phase..."/>
            </div>
            <div>
              <Lbl c="Target Date"/>
              <input style={inp} type="date" value={m.targetDate||""} disabled={locked} onChange={e=>updateMilestone(i,"targetDate",e.target.value)}/>
            </div>
            {!locked && (
              <button onClick={()=>removeMilestone(i)} style={{background:"none",border:"none",color:C.risk,cursor:"pointer",fontSize:16,paddingBottom:2,alignSelf:"end"}}>✕</button>
            )}
          </div>
          <div style={{marginTop:8}}>
            <Lbl c="Description (optional)"/>
